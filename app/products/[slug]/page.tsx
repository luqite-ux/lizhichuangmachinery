  ...
  return <div>
    ...
    <section className="border-b border-border"><div className="mx-auto grid max-w-7xl gap-9 px-4 py-12 md:grid-cols-[1fr_1.08fr] md:items-center md:px-6 md:py-16">
      ...
      <div className="grid gap-3">
        <div className="relative aspect-[4/3] overflow-hidden rounded-lg border border-border bg-[#f3f6fa]">{product.images[0] ? <Image src={product.images[0]} alt={`${product.model} ${product.name}`} fill priority sizes="(min-width: 768px) 50vw, 100vw" className="object-contain p-5" /> : null}</div>
        {product.images.length > 1 ? <div className="grid grid-cols-3 gap-3">{product.images.slice(1, 4).map((src, index) => <div key={src} className="relative aspect-[4/3] overflow-hidden rounded-md border border-border bg-[#f3f6fa]"><Image src={src} alt={`${product.model} equipment view ${index + 2}`} fill sizes="16vw" className="object-contain p-2" /></div>)}</div> : null}
      </div>
    </div></section>
